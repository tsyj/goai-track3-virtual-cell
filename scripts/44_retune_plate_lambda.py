...
REF = "current (plate .3, pxs 2)"

# BATCH_FACTORS already carries source + instrument as of 2026-08-16
GRID = [(p, x) for p in (0.15, 0.3, 0.6, 1.0, 2.0) for x in (1.0, 2.0, 6.0)]
CONFIGS = {REF: {"lam_plate": 0.3, "lam_plate_x_strain": 2.0}}
for p, x in GRID:
    if (p, x) == (0.3, 2.0):
        continue
    ...
    with Pool(N_WORKERS) as pool:
        for i, r in enumerate(pool.imap_unordered(one_job, jobs), 1):
            ...
            if i % 10 == 0 or i == len(jobs):
                ...
                pd.DataFrame(done).to_csv(os.path.join(OUT, "retune_plate_raw.csv"), index=False)
    raw = pd.DataFrame(done)
    raw.to_csv(os.path.join(OUT, "retune_plate_raw.csv"), index=False)
