...
GRID = [(0.0,1.25),(0.7,1.0),(0.7,0.75),(0.5,0.75),(0.6,0.75),(0.8,0.75),(0.5,0.6),(0.6,0.6),(0.7,0.6),(0.8,0.6),(0.5,0.5),(0.6,0.5),(0.7,0.5),(0.4,0.4),(0.5,0.4)]
...
def run(fo, P, held, tag):
    ...
    MODS = ["M1_abs(20%)","M2_rawFC(25%)","M3_ctx(20%)","M4_drug(20%)","M5_bt(10%)","M6_DEP(5%)","FC[strain_only]","FC[both]"]
    r0 = summary_row("x", evaluate(fo, expand(P, meta, allm, *ALLROWS), tag)); out = {"v21_all": r0["TOTAL"]}
    out.update({f"v21_all|{m}": r0[m] for m in MODS if m in r0})
    for b, t in GRID:
        r = summary_row("x", evaluate(fo, expand(P, meta, unseen, b, t), tag)); out[f"u_b{b}_t{t}"] = r["TOTAL"]
        out.update({f"u_b{b}_t{t}|{m}": r[m] for m in MODS if m in r})
    return out
...
d = pd.DataFrame(rows); d.to_csv(os.path.join(OUT, "unseen_expand_grid2_inner.csv"), index=False)
...
v = run(fo, P, "BAI", VAL); pd.Series(v).to_csv(os.path.join(OUT, "unseen_expand_grid2_val.csv"))
...
print("\n=== val 模块分解（vs v2.1）===")
for k in ["u_b0.7_t1.0","u_b0.7_t0.75","u_b0.6_t0.6","u_b0.5_t0.5"]:
    print(f"  {k:14s} " + "  ".join(f"{m.split('(')[0]}:{v[k+'|'+m]-v['v21_all|'+m]:+.4f}" for m in ["M1_abs(20%)","M2_rawFC(25%)","M4_drug(20%)","M5_bt(10%)","M6_DEP(5%)","FC[both]"]))
