...
REF = "incumbent (plate .3, pxs 2)"

CONFIGS = {
    "incumbent (plate .3, pxs 2)": {"booster": dict(ADOPTED),
                                    "lam": {"lam_plate": 0.3, "lam_plate_x_strain": 2.0}},
    "retuned (plate 2, pxs 6)":    {"booster": dict(ADOPTED),
                                    "lam": {"lam_plate": 2.0, "lam_plate_x_strain": 6.0}},
    "retuned + plate 1":           {"booster": dict(ADOPTED),
                                    "lam": {"lam_plate": 1.0, "lam_plate_x_strain": 6.0}},
}
...
def one_job(arg):
    ...
    um = UnifiedBackfit(
        ...
        pert_factors=[(a, c, l) for a, c, l in PERT_FACTORS],
        n_pass=6,
...
if __name__ == "__main__":
    ...
    with Pool(N_WORKERS) as pool:
        for i, r in enumerate(pool.imap_unordered(one_job, jobs), 1):
            ...
            if i % 3 == 0 or i == len(jobs):
                ...
                pd.DataFrame(done).to_csv(os.path.join(OUT, "confirm_plate_lam_raw.csv"),
                                          index=False)
    raw = pd.DataFrame(done)
    raw.to_csv(os.path.join(OUT, "confirm_plate_lam_raw.csv"), index=False)
    ...
    summ.to_csv(os.path.join(OUT, "confirm_plate_lam.csv"))
